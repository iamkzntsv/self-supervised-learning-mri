...
def make(config):
    ...
    model = MSVAE(latent_dim)
    model.train()

    # Loss function and Optimizer
    criterion = LossMSVAE(sigma=config['sigma'])
...
def train(model, train_loader, valid_loader, criterion, optimizer, config, save_model=False):
    ...
    for epoch in range(config['epochs']):
        ...
        for images in train_loader:
            ...
            optimizer.zero_grad()
            # Forward pass
            super_x_hat, x_hat, mu, log_var = model(images)
            # Calculate the loss
            loss = criterion(super_x_hat, x_hat, images, mu, log_var)
            loss.backward()
            # Update parameters
            optimizer.step()
    ...
    if save_model:
        torch.save(model.state_dict(), 'ms_vae.pt')
